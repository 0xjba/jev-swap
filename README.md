<p align="center"><img src="https://cdn.jsdelivr.net/npm/jev-swap@0.1.2/assets/banner.png" alt="jev-swap: find the decisions, run the shadow proxy, estimate the savings, see the difference, swap to Jev" width="100%"></p>
...
- Script: `video/narration.json` (`say` is spoken, `show` is the caption). `npm run voice` records each line with OpenRouter's `openai/gpt-audio` (voice `onyx`, the deepest of its voices, directed as an upbeat launch host; measured at about 112-138 Hz with 8-13 semitones of intonation so it stays lively rather than ominous), checks the transcript matches word for word, trims it, caps pauses inside a line at about 0.3 s and levels it (ffmpeg), and writes `public/vo/*.wav` plus timings; only changed lines are re-recorded.
- Music: `public/music.mp3`, a steady 118 BPM product-launch bed generated with Google Lyria 3 Pro via OpenRouter (`npm run music`, $0.08 a track) and normalized to -16 LUFS. It sits about 18 dB under the voice (13 dB between lines) and is started so its final chord lands on the logo (`MUSIC_END` in `src/Video.tsx`; check it after generating a new track).
