...
- Per call: `llm_cost = (prompt + state) * llm_in + output * llm_out`, `jev_cost = (jev_questions + state) * jev_in`, `reduction = 1 - jev_cost / llm_cost`. User input size (`state`) is unknown, so each figure is a range over `--state-min`..`--state-max` tokens (default 50..2000).
...
- Prices: `oss prices` refreshes `data/prices.json` from OpenRouter's models API (every provider's list price) and Jev's OpenRouter page; models not listed there stay unpriced.
- Speed: `oss prices` also records each used model's p50 latency and throughput from its OpenRouter page. A call's LLM time is estimated as latency + output tokens / throughput and compared with Jev at 285 ms (the midpoint of TypeSafe's reported 70-500 ms). Where Jev isn't cheaper but is within 15% of the cost and at least 1.5x faster, the call reads "about the same cost, N x faster".
...
- `dashboard.html`: the public OSS dashboard, generated from `oss-out/dashboard.json` (raw data copied to `site/data/dashboard.json`).
