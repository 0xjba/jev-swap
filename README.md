...
- Script: `video/narration.json` (`say` is spoken, `show` is the caption). `npm run voice` records each line with OpenRouter's `openai/gpt-audio` (voice `onyx`, the deepest of its voices by measured pitch), checks the transcript matches word for word, trims it, caps pauses inside a line at about 0.3 s and levels it (ffmpeg), and writes `public/vo/*.wav` plus timings; only changed lines are re-recorded.
